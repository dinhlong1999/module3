create database extra_exercise_ss03;
use extra_exercise_ss03;
...
insert into instructor(`name`,birthday, salary)
 ...
 select S.id, S.`name`, S.birthday, S.gender, S.email, S.point, S.account, C.`name` from student S
...
WHERE
 ...
 select `point`, COUNT(*) as so_luong_hoc_sinh from student
 group by `point`
 order by `point`;
 ...
 SELECT 
    `point`, COUNT(*) AS so_luong_hoc_sinh
FROM
    student
WHERE
    `point` > 5
